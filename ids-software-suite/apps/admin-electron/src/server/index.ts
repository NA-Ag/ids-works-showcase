...
// Active E2E sessions (in a real app, this might be a Map with an expiration timer)
...
export const startServer = (port: number = 3000) => {
  ...
  // In a real Electron app, this should be in app.getPath('userData')
  const dbPath = path.join(os.homedir(), '.idsworks', 'admin-suite.db');
  const { db } = initDB(dbPath);

  // Endpoint to establish the E2E encryption handshake
  ...
  // Apply E2E encryption middleware to all subsequent /api routes
  ...
  // Setup the actual CRUD routes
  setupRoutes(app, db);

  return new Promise<void>((resolve, reject) => {
    const server = app.listen(port, '0.0.0.0', () => {
      console.log(`IDS Local Admin Server running on port ${port}`);
      resolve();
    }).on('error', reject);
  });
};
