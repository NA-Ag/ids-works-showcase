...
export const startServer = (port: number = 3004) => {
  const app = express();
  app.use(cors());
  app.use(express.json());

  const dbPath = path.join(os.homedir(), '.idsworks', 'admin-suite.db'); // Same database file
  ...
  return new Promise<void>((resolve, reject) => {
    const server = app.listen(port, '0.0.0.0', () => {
      console.log(`IDS Discipline Server running on port ${port}`);
      resolve();
    }).on('error', reject);
  });
};
