    ...
    return (
        <div className="animate-fade-in p-4 md:p-10 max-w-7xl mx-auto w-full space-y-10">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b-2 border-gray-100 pb-10">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-vault-blue rounded-2xl flex items-center justify-center text-vault-yellow shadow-lg shadow-vault-blue/20">
                            <Sparkles size={24} />
                        </div>
                        <h2 className="text-3xl md:text-5xl font-black text-vault-darkBlue uppercase tracking-tighter italic">
                            {isEn ? 'Parent Portal' : 'Portal de Padres'}
                        </h2>
                    </div>
                    <div className="flex gap-4">
                        <button onClick={() => setLocalActiveTab('home')} className={`text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${localActiveTab === 'home' ? 'border-vault-blue text-vault-blue' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
                            {isEn ? 'Overview' : 'Resumen'}
                        </button>
                        <button onClick={() => setLocalActiveTab('onboarding')} className={`text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${localActiveTab === 'onboarding' ? 'border-vault-blue text-vault-blue' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
            ...
            {localActiveTab === 'home' && (
                ...
                <div className="bg-white rounded-[3rem] border-2 border-vault-blue/10 shadow-xl overflow-hidden group">
                    <div className="p-8 border-b border-gray-50 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex gap-4 items-center">
                            <div className="p-4 bg-vault-yellow rounded-2xl text-vault-darkBlue shadow-md group-hover:rotate-12 transition-transform">
                                <HelpCircle size={24} />
                            </div>
                            <div>
                                <h3 className="text-xl font-black text-vault-darkBlue uppercase tracking-tighter italic leading-none mb-1">
                        ...
                        <form onSubmit={handleAiSearch} className="relative w-full md:w-1/2">
                            <input 
                                ...
                                className="w-full pl-6 pr-16 py-5 bg-white rounded-2xl border-2 border-transparent focus:border-vault-blue outline-none shadow-inner transition-all font-medium text-sm"
                                value={aiQuery}
                                onChange={(e) => setAiQuery(e.target.value)}
                            />
                            <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 p-3 bg-vault-blue text-white rounded-xl hover:bg-vault-darkBlue transition-colors shadow-md">
                                <Send size={18} />
                            </button>
                        </form>
                    </div>
                    ...
                    <div className={`p-8 bg-white transition-all duration-500 overflow-hidden ${aiResponse || isAiLoading ? 'max-h-96' : 'max-h-0'}`}>
                        {isAiLoading ? (
                            ...
                            <div className="flex gap-6 items-start bg-vault-yellow/5 p-8 rounded-3xl border border-vault-yellow/20">
                                <div className="p-3 bg-vault-yellow rounded-xl text-vault-darkBlue shrink-0">
                                    <Sparkles size={20} />
                                </div>
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest text-vault-blue border-l border-gray-200 pl-4">{isEn ? 'Sourced from Student Handbook' : 'Fuente: Reglamento Estudiantil'}</span>
                                    </div>
                                </div>
                            </div>
                        )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-[3rem] shadow-xl border border-gray-100 overflow-hidden">
                        <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
                            <div className="flex gap-4 items-center">
                                <Calendar size={20} className="text-vault-blue" />
                                <h3 className="font-black text-vault-darkBlue uppercase tracking-widest text-xs">
                            ...
                            {[
                                ...
                                <div key={i} className="flex items-center justify-between group cursor-pointer p-4 hover:bg-gray-50 rounded-[1.5rem] transition-all border border-transparent hover:border-gray-100">
                                    <div className="flex gap-6 items-center">
                                        <div className="w-16 h-16 bg-white border border-gray-100 rounded-2xl flex flex-col items-center justify-center shadow-sm group-hover:bg-vault-blue group-hover:text-white transition-all">
                                        ...
                                        <div>
                                            <p className="text-lg font-black text-vault-darkBlue group-hover:text-vault-blue transition-colors uppercase tracking-tighter italic leading-none mb-2">{b.title}</p>
                                            <div className="flex gap-4 items-center">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">{b.child}</span>
                                                <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${b.status === 'Pending' ? 'bg-[#8B0000]/10 text-[#8B0000]' : 'bg-vault-blue/10 text-vault-blue'}`}>{b.status}</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-vault-darkBlue rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-2xl">
                            <div className="relative z-10 space-y-8">
                                ...
                                <button 
                                    ...
                                    className="w-full py-4 bg-vault-yellow text-vault-darkBlue rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-white transition-all shadow-lg hover:-translate-y-1"
                                >
                        ...
                        {showDigitalId && (
                            <div className="fixed inset-0 bg-vault-darkBlue/90 backdrop-blur-md z-[200] flex items-center justify-center p-4 animate-fade-in">
                                <div className="bg-white w-full max-w-sm rounded-[3rem] overflow-hidden shadow-2xl relative">
                                    <button onClick={() => setShowDigitalId(false)} className="absolute top-6 right-6 text-gray-400 hover:text-vault-darkBlue"><X size={24} /></button>
                                    <div className="bg-vault-blue p-8 text-center text-white">
                                        <div className="w-24 h-24 bg-white rounded-full mx-auto mb-4 border-4 border-vault-yellow flex items-center justify-center text-vault-darkBlue">
                                            <User size={48} />
                                        </div>
                                    ...
                                    <div className="p-10 text-center space-y-6">
                                        <div className="bg-gray-50 p-6 rounded-3xl border-2 border-dashed border-gray-200 inline-block">
                                            <QrCode size={160} className="text-vault-darkBlue" />
                                        </div>
                                        <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                ...
                                { icon: CheckCircle2, label: isEn ? 'Grades' : 'Calificaciones', color: 'bg-vault-yellow/10 text-vault-darkBlue' }
                            ].map((item, i) => (
            ...
            {localActiveTab === 'onboarding' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
                    <div className="space-y-8">
                        <div className="bg-white p-10 rounded-[3rem] border-2 border-vault-blue/5 shadow-xl space-y-6">
                            <h3 className="text-2xl font-black text-vault-darkBlue uppercase tracking-tighter italic border-b border-gray-50 pb-4">
                            ...
                            <div className="space-y-4">
                                {onboardingDocs.map(doc => (
                                    <div key={doc.id} className="p-6 bg-gray-50 rounded-2xl flex items-center justify-between group hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-vault-blue/10">
                                        <div className="flex gap-4 items-center">
                                            <div className="p-3 bg-white rounded-xl text-gray-400 group-hover:text-vault-blue">
                                                <FileText size={20} />
                                            </div>
                                            <div>
                                        ...
                                        {doc.status !== 'Verified' && (
                                            <button 
                                                ...
                                                className="px-6 py-2 bg-vault-blue text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-vault-darkBlue transition-all disabled:bg-gray-200"
                                            >
                    ...
                    <div className="space-y-8">
                        <div className="bg-vault-darkBlue p-10 rounded-[3rem] text-white space-y-8 relative overflow-hidden group">
