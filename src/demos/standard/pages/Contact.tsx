    ...
    >
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <p className="text-2xl text-vault-darkBlue font-black uppercase tracking-tighter italic leading-tight border-l-4 border-vault-yellow pl-8">
                "{content.intro}"
            </p>
        </section>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
            ...
            <div className="lg:w-1/3 space-y-8">
                <div className="flex items-center gap-4 border-b-2 border-vault-yellow pb-4 mb-8">
                    <Building2 size={24} className="text-vault-blue" />
                    <h3 className="text-xl font-black text-vault-darkBlue uppercase tracking-widest italic">{content.infoTitle}</h3>
                </div>
                
                <div className="grid grid-cols-1 gap-4">
                    {content.info.map((item, idx) => (
                        <div key={idx} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex items-start gap-6 hover:shadow-md transition-all group border-b-4 hover:border-vault-blue">
                            <div className="bg-vault-paper p-3 rounded-xl text-vault-blue group-hover:bg-vault-darkBlue group-hover:text-white transition-colors">
                                <item.icon size={20} />
                            </div>
                            <div>
                                <h5 className="font-black text-vault-darkBlue text-[10px] uppercase tracking-[0.2em] mb-2 opacity-40">{item.label}</h5>
                ...
                {/* Secure Node Info */}
                <div className="bg-vault-darkBlue p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden group border-b-8 border-idsRed">
            ...
            <div className="lg:w-2/3">
                <div className="bg-vault-paper p-1 border-2 border-vault-blue/10 rounded-[3rem] shadow-2xl">
                    <div className="bg-white p-10 md:p-16 rounded-[2.8rem] space-y-10">
                        <div className="text-center space-y-4">
                            <h3 className="text-3xl font-black text-vault-darkBlue uppercase tracking-tighter italic">{content.formTitle}</h3>
                        ...
                        {submitted ? (
                            <div className="text-center space-y-8 animate-scale-up py-12">
                                ...
                                <div className="space-y-4">
                                    <h4 className="text-2xl font-black text-vault-darkBlue uppercase tracking-tighter italic">Message Logged</h4>
                                ...
                                <button onClick={() => setSubmitted(false)} className="text-vault-blue font-black text-[10px] uppercase tracking-widest hover:underline">New Communication</button>
                            </div>
                        ) : (
                            <form className="space-y-8" onSubmit={handleFormSubmit}>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea required rows={5} className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue resize-none"></textarea>
                                </div>
                                <button 
