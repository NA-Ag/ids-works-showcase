...
const Accreditations: React.FC = () => {
  ...
  return (
    ...
    >
        ...
        {items.map((item) => (
            <div key={item.name} className="flex flex-col bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 hover:shadow-2xl transition-all duration-500 group border-b-8 hover:border-vault-yellow">
                ...
                <div className="flex-grow flex flex-col">
                    <h4 className="text-2xl font-black text-vault-darkBlue uppercase tracking-tight mb-2 italic">{item.name}</h4>
                    <p className="text-[10px] text-vault-blue font-black uppercase tracking-widest mb-6 opacity-60">{item.fullName}</p>
      ...
      <div className="mt-20 p-12 bg-vault-darkBlue rounded-[3rem] text-white shadow-2xl relative overflow-hidden text-center border-b-8 border-vault-yellow">
