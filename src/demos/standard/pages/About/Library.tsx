    ...
    >
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="relative py-10">
            ...
            <p className="text-3xl md:text-5xl font-black text-vault-darkBlue text-center max-w-4xl mx-auto leading-tight pt-12 uppercase tracking-tighter italic">
                "{content.quote}"
            </p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-16">
                {data.library.stats.map((stat: any, idx: number) => (
                    <div key={idx} className="bg-vault-paper p-8 rounded-[2rem] border border-vault-blue/10 shadow-sm text-center hover:border-vault-yellow transition-all group">
                        <p className="text-4xl font-black text-vault-blue group-hover:scale-110 transition-transform italic tracking-tighter">{stat.value}</p>
        ...
        <section className="bg-white p-8 md:p-16 rounded-[4rem] border border-vault-blue/10 shadow-xl overflow-hidden relative">
            ...
            <div className="relative z-10 space-y-12">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-gray-100 pb-12">
                    <div className="space-y-2 text-center md:text-left">
                        <h3 className="text-3xl font-black text-vault-darkBlue uppercase tracking-tighter italic">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                        <div className="relative group">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-vault-blue transition-colors" size={18} />
                            <input 
                                type="text" 
                                placeholder={isEn ? "Search catalog..." : "Buscar en catálogo..."}
                                className="pl-12 pr-6 py-4 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-vault-blue focus:bg-white outline-none w-full sm:w-80 transition-all font-medium text-sm"
                                value={searchQuery}
                        ...
                        <div className="flex gap-2 p-1 bg-gray-100 rounded-2xl shrink-0">
                            {['all', 'physical', 'digital', 'video'].map(f => (
                                <button 
                                    ...
                                    className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-vault-blue text-white shadow-md' : 'text-gray-400 hover:text-gray-600'}`}
                                >
                                    {f}
                                </button>
                            ))}
                    ...
                    {filteredResources.map((res, i) => (
                        <div key={i} className="group bg-gray-50 p-8 rounded-[2rem] border-2 border-transparent hover:border-vault-yellow hover:bg-white transition-all duration-500 shadow-sm hover:shadow-2xl">
                            <div className="flex justify-between items-start mb-6">
                                <div className={`p-4 rounded-2xl ${res.type === 'Physical' ? 'bg-vault-blue text-white' : res.type === 'Digital' ? 'bg-emerald-600 text-white' : 'bg-[#8B0000] text-white'} transition-transform group-hover:rotate-12`}>
                                    {res.type === 'Physical' ? <LibraryIcon size={20} /> : res.type === 'Digital' ? <Globe size={20} /> : <BookOpen size={20} />}
                                </div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-300 group-hover:text-vault-blue transition-colors">{res.type}</span>
                            </div>
                            <h4 className="text-xl font-black text-vault-darkBlue uppercase tracking-tighter italic leading-tight mb-2 group-hover:text-vault-blue transition-colors">{res.title}</h4>
                            ...
                            <div className="flex justify-between items-end border-t border-gray-100 pt-6">
                                <div>
                                    ...
                                    <span className="text-xs font-bold text-vault-darkBlue">{res.cat}</span>
                                </div>
                                <button className="w-10 h-10 bg-white border border-gray-100 rounded-full flex items-center justify-center text-gray-400 hover:bg-vault-yellow hover:text-vault-darkBlue hover:border-vault-yellow transition-all">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8 order-2 lg:order-1">
                <div className="flex items-center gap-4 mb-4 border-b-4 border-vault-yellow pb-4 w-fit">
                    <div className="bg-vault-blue p-3 rounded-xl text-white">
        ...
        {/* Services Grid */}
        <section className="bg-vault-blue -mx-4 lg:-mx-12 px-4 lg:px-12 py-24 text-white relative overflow-hidden border-b-8 border-vault-yellow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {data.library.services.map((service: any, idx: number) => (
                        <div key={idx} className="bg-white/10 p-10 rounded-[2.5rem] border border-white/20 hover:bg-white/20 transition-all group flex gap-6 items-start">
                            <div className="bg-vault-yellow p-4 rounded-2xl text-vault-darkBlue shrink-0 group-hover:rotate-12 transition-transform">
                                <Search size={24} />
                            </div>
                            <div>
        ...
        {/* Final CTA */}
        <div className="bg-vault-paper p-12 md:p-24 rounded-[4rem] text-center shadow-inner relative overflow-hidden border border-vault-blue/10">
            <div className="relative z-10 space-y-10 max-w-3xl mx-auto">
                <div className="space-y-6">
                    <Award className="w-12 h-12 text-vault-blue mx-auto opacity-50" />
                ...
                <div className="pt-8">
                    <a 
                        ...
                        className="inline-block bg-vault-blue text-white px-12 py-5 rounded-sm font-black uppercase tracking-widest text-sm hover:bg-vault-darkBlue hover:-translate-y-1 transition-all shadow-xl"
                    >
