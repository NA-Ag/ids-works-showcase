  ...
  return (
    <PageLayout title={content.title} breadcrumbs={content.breadcrumbs}>
      <div className="max-w-4xl mx-auto space-y-12 py-12">
        ...
        {/* Protection Banner */}
        <div className="bg-vault-darkBlue rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden border-b-8 border-vault-yellow">
            <div className="relative z-10 flex flex-col items-center text-center space-y-8">
                <div className="bg-vault-yellow p-4 rounded-2xl text-vault-darkBlue shadow-lg">
                    <Lock size={40} />
                </div>
                ...
                <button 
                    onClick={() => navigate('/demos/standard/login')}
                    className="group bg-white text-vault-darkBlue px-12 py-5 rounded-xl font-black uppercase tracking-widest text-sm hover:bg-vault-yellow transition-all flex items-center gap-4 shadow-xl"
                >
                    <LogIn size={20} />
                    {content.loginBtn}
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center px-6">
            <div className="space-y-8">
                <div className="flex items-center gap-4">
                    <CalendarIcon className="text-vault-blue w-8 h-8" />
                    <h3 className="text-2xl font-black text-vault-darkBlue uppercase tracking-tight">{content.previewTitle}</h3>
                </div>
                <div className="space-y-4">
                    {content.mockEvents.map((ev, i) => (
                        <div key={i} className="flex items-center gap-6 p-4 bg-gray-50 rounded-xl border border-gray-100 group hover:border-vault-yellow transition-colors">
                            <span className="font-mono font-black text-vault-blue text-sm whitespace-nowrap">{ev.date}</span>
