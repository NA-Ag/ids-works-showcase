...
const Admissions: React.FC = () => {
  ...
  const content = {
    ...
    processSteps: [
        { title: isEn ? "Inquiry" : "Consulta", icon: Mail, color: "bg-vault-darkBlue" },
        { title: isEn ? "Interview" : "Entrevista", icon: MessageSquare, color: "bg-vault-blue" },
        { title: isEn ? "Tour" : "Visita", icon: School, color: "bg-vault-blue" },
        { title: isEn ? "Assessment" : "Evaluación", icon: FileSearch, color: "bg-vault-darkBlue" },
        { title: isEn ? "Review" : "Revisión", icon: ClipboardCheck, color: "bg-vault-blue" },
        { title: isEn ? "Enrollment" : "Inscripción", icon: CheckCircle2, color: "bg-vault-blue" }
    ],
    testimonials: [
        {
    ...
    <PageLayout title={content.title} breadcrumbs={content.breadcrumbs} bannerImage="https://images.unsplash.com/photo-1524178232363-1fb2b075b655?auto=format&fit=crop&q=80&w=2000">
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <p className="text-3xl font-black text-vault-darkBlue uppercase tracking-tighter italic leading-tight">
        ...
        <section className="bg-gray-50 -mx-4 lg:-mx-12 px-4 lg:px-12 py-24 border-y border-gray-100 relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                <h3 className="text-4xl font-black text-vault-darkBlue uppercase tracking-tighter italic text-center mb-20">{content.processTitle}</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
                    {content.processSteps.map((step, idx) => (
                        <div key={idx} className="flex flex-col items-center group">
                            ...
                            <h4 className="text-sm font-black text-vault-darkBlue uppercase tracking-widest text-center">{step.title}</h4>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-12">
                <div className="flex items-center gap-4 border-b-4 border-vault-yellow pb-4 w-fit">
                    <h3 className="text-3xl font-black text-vault-darkBlue uppercase tracking-tighter italic">{content.policiesTitle}</h3>
                </div>
                <div className="grid grid-cols-1 gap-6">
                    {data.admissions.policies.map((policy: any, idx: number) => (
                        <div key={idx} className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex gap-6">
                            <div className="bg-vault-paper p-3 rounded-xl text-vault-blue h-fit">
                                <Users size={20} />
                            </div>
                            <div>
                                <h4 className="font-black text-vault-darkBlue uppercase tracking-tight mb-1 italic">{policy.title}</h4>
            ...
            <div className="bg-vault-darkBlue p-12 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border-b-8 border-vault-yellow">
                ...
                <ul className="space-y-6 relative z-10">
                    {data.admissions.requirements.map((req: string, idx: number) => (
                        <li key={idx} className="flex items-center gap-4 group">
                            <div className="bg-white/10 p-2 rounded-lg group-hover:bg-vault-yellow group-hover:text-vault-darkBlue transition-colors">
        ...
        <section id="testimonials" className="py-12 scroll-mt-24">
            <div className="text-center mb-16">
                <h3 className="text-4xl font-black text-vault-darkBlue uppercase tracking-tighter italic mb-4">{isEn ? "Scholarly Testimonials" : "Testimonios Académicos"}</h3>
                <div className="h-1 w-24 bg-idsRed mx-auto"></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
                {content.testimonials.map((t, i) => (
                    <div key={i} className="bg-vault-paper p-12 rounded-[3rem] border border-vault-blue/10 relative group">
                        <div className="text-idsRed opacity-20 absolute top-8 left-8"><FileText size={48} /></div>
                        <p className="text-xl text-vault-darkBlue font-medium leading-relaxed italic mb-8 relative z-10">"{t.text}"</p>
                        <div className="pt-6 border-t border-vault-blue/5">
                            <h5 className="font-black text-vault-darkBlue uppercase tracking-widest text-xs">{t.name}</h5>
        ...
        <section className="max-w-4xl mx-auto bg-white rounded-[4rem] shadow-2xl border border-gray-100 overflow-hidden">
            <div className="bg-vault-darkBlue p-12 text-center text-white relative">
                ...
                {submitted ? (
                    <div className="text-center space-y-8 animate-scale-up">
                        ...
                        <div className="space-y-4">
                            <h4 className="text-3xl font-black text-vault-darkBlue uppercase tracking-tighter italic">Enquiry Received</h4>
                        ...
                        <button onClick={() => setSubmitted(false)} className="text-vault-blue font-black text-xs uppercase tracking-widest hover:underline">Submit another enquiry</button>
                    </div>
                ) : (
                    <form onSubmit={handleFormSubmit} className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                ...
                                <input required type="text" className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Email Address</label>
                                <input required type="email" className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Stage of Interest</label>
                                <select className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue appearance-none">
                            ...
                            <div className="space-y-2">
                                ...
                                <input required type="date" className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Additional Information</label>
                            <textarea rows={4} className="w-full bg-gray-50 border-2 border-transparent focus:border-vault-blue focus:bg-white p-4 rounded-xl outline-none transition-all font-bold text-vault-darkBlue resize-none" />
                        </div>
                        <button 
                            disabled={isSubmitting}
                            type="submit" 
                            className="w-full bg-vault-darkBlue text-white py-6 rounded-xl font-black uppercase tracking-[0.2em] text-sm hover:bg-vault-blue transition-all shadow-2xl flex items-center justify-center gap-4 group disabled:opacity-50"
                        >
                            {isSubmitting ? (
