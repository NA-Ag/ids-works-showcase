...
export const QuickViewModal: React.FC<QuickViewModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-8">
      <div className="absolute inset-0 bg-vault-darkWine/80 backdrop-blur-md animate-fade-in" onClick={onClose}></div>
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-scale-up border-t-8 border-vault-yellow">
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 p-2 text-gray-400 hover:text-vault-darkWine hover:bg-gray-100 rounded-full transition-all z-10"
        >
          <X size={24} />
        </button>
        ...
        <div className="flex flex-col">
          ...
          <div className="p-10 space-y-8">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    <CheckCircle2 size={18} className="text-vault-wine shrink-0 mt-0.5" />
                    <span className="text-sm font-bold text-vault-darkWine leading-tight uppercase tracking-tight">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4">
              <button 
                onClick={onClose}
                className="flex-grow bg-vault-darkWine text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-vault-wine transition-all"
              >
                Close Preview
              </button>
              <a 
                href="/demos/integrated/admissions"
                className="flex-grow bg-vault-yellow text-vault-darkWine py-4 rounded-xl font-black uppercase tracking-widest text-xs text-center hover:bg-white border-2 border-transparent hover:border-vault-yellow transition-all"
              >
