...
export const ColegioIDSLogo: React.FC<{ className?: string, inverted?: boolean }> = ({ className = "h-12", inverted = false }) => {
  const primary = inverted ? "#FFD700" : "#4A0404"; // Vault Yellow or Dark Wine
  const secondary = inverted ? "#FFFFFF" : "#722F37"; // White or Vault Wine

  return (
    <div className={`flex items-center gap-4 ${className}`}>
      <svg viewBox="0 0 100 100" className="h-full w-auto drop-shadow-lg">
      ...
      <div className="flex flex-col justify-center leading-none">
        <span className={`font-serif font-black tracking-tighter text-2xl md:text-3xl ${inverted ? 'text-white' : 'text-vault-darkWine'}`}>COLEGIO</span>
        <span className={`font-mono font-bold tracking-[0.4em] text-[10px] md:text-xs ${inverted ? 'text-vault-yellow' : 'text-vault-wine'}`}>IDS WORKS</span>
      </div>
    </div>
  );
};
