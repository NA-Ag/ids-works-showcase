      ...
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="space-y-12">
            <div className="flex flex-col md:flex-row gap-8 items-center bg-white p-10 rounded-[3rem] border border-gray-100 shadow-xl border-b-8 border-vault-yellow relative overflow-hidden group">
                ...
                <div className="bg-vault-darkWine p-6 rounded-2xl shrink-0 shadow-lg relative z-10">
                    <ShieldCheck className="w-12 h-12 text-vault-yellow" />
                </div>
                <p className="text-2xl text-vault-darkWine leading-relaxed font-light italic relative z-10 border-l-4 border-vault-paper pl-8">
                    "{content.intro}"
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-vault-wine/5 p-8 rounded-[2rem] border border-vault-wine/10 flex items-center gap-6">
                    <div className="bg-vault-wine p-3 rounded-xl text-white shadow-lg">
                        <History size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-vault-wine uppercase tracking-widest mb-1">{isEn ? 'Last Inspection' : 'Última Inspección'}</p>
                        <p className="text-2xl font-black text-vault-darkWine uppercase italic tracking-tighter">{data.inspections.lastVisit}</p>
                    </div>
                </div>
                <div className="bg-vault-paper p-8 rounded-[2rem] border border-vault-wine/10 flex items-center gap-6">
                    <div className="bg-vault-darkWine p-3 rounded-xl text-vault-yellow shadow-lg">
                        <Award size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-vault-darkWine uppercase tracking-widest mb-1">{isEn ? 'Certifications' : 'Certificaciones'}</p>
                        <p className="text-2xl font-black text-vault-wine uppercase italic tracking-tighter">100% Active</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section className="space-y-12">
            ...
            <div className="grid grid-cols-1 gap-8">
                {data.inspections.reports.map((report: any, idx: number) => (
                    <div key={idx} className="group bg-white border border-gray-100 rounded-[2.5rem] p-10 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8 hover:shadow-xl transition-all border-b-8 hover:border-vault-wine">
                        <div className="flex items-center gap-8">
                            <div className="bg-vault-paper p-6 rounded-3xl text-vault-wine group-hover:bg-vault-darkWine group-hover:text-white transition-colors border border-vault-wine/5">
                                <FileText className="w-10 h-10" />
                            </div>
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <h4 className="text-2xl font-black text-vault-darkWine uppercase tracking-tighter italic">{report.title}</h4>
                                    <span className="bg-vault-yellow text-vault-darkWine text-[10px] font-black px-3 py-1 rounded-sm uppercase tracking-widest">{report.year}</span>
                                </div>
                                <div className="flex items-center gap-2 mb-4">
                                    <p className="text-[10px] text-vault-wine font-black uppercase tracking-[0.2em]">{report.org}</p>
                        ...
                        <div className="flex flex-col gap-3">
                            <button className="flex items-center justify-center gap-3 px-6 py-3 bg-vault-paper rounded-xl text-vault-wine font-black text-[10px] uppercase tracking-widest border border-vault-wine/10 hover:bg-vault-wine hover:text-white transition-all shadow-sm">
        ...
        {/* Closing CTA */}
        <div className="bg-vault-darkWine text-white p-12 md:p-24 rounded-[4rem] text-center shadow-2xl relative overflow-hidden border-b-8 border-vault-yellow">
            ...
            <div className="relative z-10 space-y-10 max-w-3xl mx-auto">
                ...
                <div className="pt-8">
                    <a 
                        ...
                        className="inline-block bg-vault-yellow text-vault-darkWine px-12 py-5 rounded-sm font-black uppercase tracking-widest text-sm hover:bg-white hover:-translate-y-1 transition-all shadow-xl"
                    >
                        {content.cta.btn}
                    </a>
                </div>
