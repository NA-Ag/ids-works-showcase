...
const Login: React.FC = () => {
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    if (step === 1 && email) {
      ...
      if (email.includes('alumni') || email.includes('egresado')) role = 'alumni';
      navigate('/demos/integrated/dashboard', { state: { role, email } });
    }
  };
  ...
  const demoAccounts = [
    ...
    { role: isEn ? "Alumni" : "Egresado", email: "alumni@colegioids.edu.mx" }
  ];
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 font-sans relative overflow-hidden" style={{
      ...
      <div className="absolute top-8 left-8 z-50 flex items-center gap-4">
        <Link to="/demos/integrated" className="bg-white/80 backdrop-blur-md px-6 py-2.5 rounded-full shadow-lg border border-gray-200 text-vault-darkWine flex items-center gap-2 hover:bg-white transition-all font-black uppercase tracking-widest text-[10px]">
            <ChevronLeft size={14} /> {translations.backBtn}
        </Link>
        <button onClick={() => setShowInfo(!showInfo)} className="bg-vault-darkWine text-white p-2.5 rounded-full shadow-lg hover:bg-vault-wine transition-all relative">
            <HelpCircle size={18} />
            {showInfo && (
                <div className="absolute top-full left-0 mt-4 w-72 bg-white p-6 rounded-2xl shadow-2xl border border-gray-100 text-left animate-fade-in-up">
                    <h4 className="font-black text-vault-darkWine uppercase tracking-tight text-xs mb-3">{translations.infoTitle}</h4>
        ...
        {showCreds && (
            <div className="bg-white p-6 rounded-2xl shadow-2xl border-2 border-vault-yellow w-80 animate-fade-in-up mb-2">
                <div className="flex justify-between items-center mb-4">
                    <h4 className="font-black text-vault-darkWine uppercase tracking-tight text-sm flex items-center gap-2"><Info size={16} className="text-vault-wine" /> {translations.demoBtn}</h4>
                    <button onClick={() => setShowCreds(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
                </div>
                <div className="space-y-4">
                    {demoAccounts.map(acc => (
                        <div key={acc.email} className="p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-vault-wine transition-colors cursor-pointer group" onClick={() => { setEmail(acc.email); setPassword('123Demo'); if(step === 2) setShowCreds(false); }}>
                            <p className="text-[10px] font-black text-vault-wine uppercase tracking-widest mb-1">{acc.role}</p>
                            <p className="text-xs font-mono text-gray-600 break-all">{acc.email}</p>
                        </div>
                    ))}
                    <div className="pt-2 border-t border-gray-100"><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Password for all:</p><p className="text-xs font-mono font-bold text-vault-darkWine">123Demo</p></div>
                </div>
            </div>
        )}
        <button onClick={() => setShowCreds(!showCreds)} className="bg-vault-darkWine text-white px-6 py-3 rounded-full shadow-2xl border-2 border-vault-yellow flex items-center gap-3 hover:bg-vault-yellow hover:text-vault-darkWine transition-all group active:scale-95"><Key size={18} className="group-hover:rotate-12 transition-transform" /><span className="font-black uppercase tracking-widest text-xs">{translations.demoBtn}</span></button>
      </div>
