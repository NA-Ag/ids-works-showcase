...
import { AdmissionsSync } from '../components/dashboard/AdmissionsSync';
...
const Dashboard: React.FC = () => {
  ...
  const getTeamsList = () => {
    if (originalRole === 'admin') return [{ id: 'ids', name: "Colegio IDS", sub: "Global Admin", color: "bg-vault-darkWine" }];
    if (originalRole === 'teacher') return [
        { id: '10a', name: "Year 10A - Science", sub: "Class Group", color: "bg-emerald-600" },
        { id: 'faculty', name: "Faculty Hub", sub: "Staff Room", color: "bg-vault-wine" },
    ...
    return (
        <div className="flex flex-col h-screen bg-[#f8fafc] font-sans overflow-hidden">
            <MicrosoftTopBar 
                ...
                onLogout={() => navigate('/demos/integrated/login')}
            ...
            <main className="flex-grow overflow-y-auto p-6 md:p-12 space-y-10 max-w-6xl mx-auto w-full">
                <header className="space-y-2">
                    <h2 className="text-4xl font-black text-vault-darkWine uppercase tracking-tighter italic">{isEn ? 'Institutional Networking' : 'Red Institucional'}</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        {/* Featured Mentorship/Events */}
                        <div className="bg-white rounded-[3rem] border border-gray-100 shadow-xl overflow-hidden group">
                            <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-vault-wine/5">
                                <h3 className="text-xl font-black text-vault-darkWine uppercase tracking-tighter italic flex items-center gap-3">
                                    <Activity size={20} className="text-vault-wine" /> {isEn ? 'Mentorship & Events' : 'Mentoría y Eventos'}
                                </h3>
                                <button className="text-[10px] font-black uppercase tracking-widest text-vault-wine hover:text-vault-darkWine">{isEn ? 'View All' : 'Ver Todos'}</button>
                            </div>
                            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:border-vault-wine/30 transition-all cursor-pointer">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md mb-3 inline-block">{isEn ? 'Virtual Session' : 'Sesión Virtual'}</span>
                                    <h4 className="font-bold text-gray-800 mb-2">{isEn ? 'Tech Industry Q&A' : 'Q&A Industria Tecnológica'}</h4>
                                    <p className="text-xs text-gray-500 mb-4">{isEn ? 'Hosted by Class of 2015' : 'Presentado por Generación 2015'}</p>
                                    <div className="flex items-center gap-2 text-xs font-bold text-vault-wine">
                                        <Calendar size={14} /> 28 Feb 2026
                                    </div>
                                </div>
                                <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:border-vault-wine/30 transition-all cursor-pointer">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-orange-600 bg-orange-50 px-2 py-1 rounded-md mb-3 inline-block">{isEn ? 'Mentorship' : 'Mentoría'}</span>
                                    <h4 className="font-bold text-gray-800 mb-2">{isEn ? 'University Prep Program' : 'Programa Prep. Universitaria'}</h4>
                                    <p className="text-xs text-gray-500 mb-4">{isEn ? 'Seeking mentors for current seniors' : 'Buscando mentores para alumnos de último año'}</p>
                                    <div className="flex items-center gap-2 text-xs font-bold text-vault-wine">
                                        <Users size={14} /> {isEn ? 'Apply Now' : 'Aplicar Ahora'}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Job Board */}
                        <div className="bg-white p-8 md:p-10 rounded-[3rem] border border-gray-100 shadow-xl space-y-6">
                            <div className="flex justify-between items-center border-b border-gray-50 pb-4">
                                <h3 className="text-lg font-black text-vault-darkWine uppercase tracking-widest">{isEn ? 'Internal Job Board' : 'Bolsa de Trabajo'}</h3>
                                <button className="px-4 py-2 bg-gray-50 text-gray-600 text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-vault-wine hover:text-white transition-colors">
                                    {isEn ? 'Post a Job' : 'Publicar Vacante'}
                                </button>
                            </div>
                            <div className="space-y-4">
                                {[
                                    { title: "Senior Software Engineer", company: "TechCorp Mexico", type: "Full-time", posted: "2d ago" },
                                    { title: "Academic Coordinator", company: "Colegio IDS", type: "Full-time", posted: "1w ago" },
                                    { title: "Marketing Intern", company: "Global Brands", type: "Internship", posted: "3w ago" }
                                ].map((job, i) => (
                                    <div key={i} className="p-5 bg-gray-50 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group cursor-pointer hover:bg-white border border-transparent hover:border-vault-wine/20 hover:shadow-md transition-all">
                                        <div className="flex gap-4 items-center">
                                            <div className="p-3 bg-white border border-gray-100 rounded-xl text-gray-400 group-hover:text-vault-wine">
                                                <Briefcase size={20} />
                                            </div>
                                            <div>
                                                <p className="font-bold text-gray-800">{job.title}</p>
                                                <div className="flex items-center gap-3 mt-1">
                                                    <p className="text-xs text-vault-wine font-bold">{job.company}</p>
                                                    <span className="text-[10px] font-mono text-gray-400">• {job.type}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                                            <span className="text-[10px] font-bold text-gray-400">{job.posted}</span>
                                            <ChevronRight size={16} className="text-gray-300 group-hover:text-vault-wine transition-colors group-hover:translate-x-1" />
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-vault-darkWine p-8 md:p-10 rounded-[3rem] text-white flex flex-col justify-between relative overflow-hidden group shadow-2xl">
                            <div className="relative z-10 space-y-6">
                                <div className="p-3 bg-vault-yellow/20 text-vault-yellow w-fit rounded-xl backdrop-blur-sm">
                                    <Users size={24} />
                                </div>
                                <div>
                                    <h3 className="text-2xl font-black text-vault-yellow uppercase tracking-tighter italic mb-2">{isEn ? 'Alumni Directory' : 'Directorio'}</h3>
                                    <p className="text-sm text-blue-100 opacity-80 leading-relaxed">{isEn ? 'Search for alumni by year, major, or industry to expand your network.' : 'Busque egresados por año, carrera o industria para expandir su red.'}</p>
                                </div>
                                <button className="w-full py-3 bg-vault-yellow text-vault-darkWine rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-white transition-colors shadow-lg hover:-translate-y-0.5">
                                    {isEn ? 'Access Directory' : 'Acceder al Directorio'}
                                </button>
                            </div>
                            <Globe className="absolute right-[-40px] bottom-[-40px] w-64 h-64 opacity-10 group-hover:scale-110 transition-transform duration-700 rotate-12" />
                        </div>
                    </div>
                </div>
            </main>
        ...
        <div className="flex flex-grow overflow-hidden relative">
          <aside className={`fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <nav className="py-4 px-2 space-y-1">
              {['home', 'admissions', 'security', 'cms', 'health', 'settings'].map(id => (
                <div key={id} onClick={() => setActiveAdminTab(id)} className={`flex items-center gap-4 p-3 rounded-md cursor-pointer ${activeAdminTab === id ? 'bg-blue-50 text-[#0078d4] font-bold border-l-4 border-[#0078d4]' : 'text-gray-600 hover:bg-gray-100'}`}>
                  {id === 'home' && <Layout size={18} />} {id === 'admissions' && <Users size={18} />} {id === 'security' && <Shield size={18} />} {id === 'cms' && <FileText size={18} />} {id === 'health' && <Activity size={18} />} {id === 'settings' && <Settings size={18} />}
