...
const Publications: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="space-y-12">
            <div className="flex items-center gap-4 border-b-4 border-vault-yellow pb-4 w-fit">
                <BookOpen size={32} className="text-vault-wine" />
                <h3 className="text-3xl font-black text-vault-darkWine uppercase tracking-tighter italic">{content.magazinesTitle}</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {data.publications.magazines.map((mag: any, idx: number) => (
                    <div key={idx} className="bg-vault-paper p-10 rounded-[3rem] border border-vault-wine/10 shadow-xl flex flex-col justify-between relative group hover:border-idsRed transition-all">
                        <div className="absolute top-8 right-10 text-vault-wine/5 group-hover:text-idsRed/5 transition-colors">
                            <Bookmark size={120} fill="currentColor" />
                        </div>
                        <div className="relative z-10 space-y-6">
                            <div>
                                <span className="bg-vault-darkWine text-white text-[10px] font-black px-3 py-1 rounded-sm uppercase tracking-widest">{mag.type}</span>
                                <h4 className="text-3xl font-black text-vault-darkWine uppercase tracking-tighter italic mt-4">{mag.title}</h4>
                        ...
                        <button 
                            ...
                            className="relative z-10 mt-10 flex items-center gap-3 bg-white border border-gray-200 p-4 rounded-xl text-vault-wine font-black text-[10px] uppercase tracking-widest hover:bg-vault-wine hover:text-white transition-all shadow-sm"
                        >
        ...
        <div className="flex flex-col lg:flex-row gap-16">
            ...
            <div className="lg:w-2/3 space-y-16">
                <div className="flex items-center gap-4 border-b-4 border-vault-yellow pb-4 w-fit">
                    <FileText size={32} className="text-vault-wine" />
                    <h3 className="text-3xl font-black text-vault-darkWine uppercase tracking-tighter italic">{content.newsTitle}</h3>
                </div>
                    ...
                    {data.publications.news.map((post: any, idx: number) => (
                        <article key={idx} className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-100 hover:shadow-2xl transition-all duration-500 flex flex-col md:flex-row border-b-8 hover:border-idsRed">
                            <div className="md:w-2/5 relative overflow-hidden">
                                ...
                                />
                                <div className="absolute top-6 left-6 bg-vault-yellow text-vault-darkWine px-4 py-1 rounded-sm text-[10px] font-black uppercase tracking-widest shadow-lg">
                                    {post.category}
                                </div>
                            </div>
                            <div className="md:w-3/5 p-10 space-y-6">
                                ...
                                <h3 className="text-3xl font-black text-vault-darkWine uppercase tracking-tighter leading-tight italic">
                                    {post.title}
                                </h3>
                                ...
                                <button 
                                    onClick={triggerAlert}
                                    className="inline-flex items-center text-vault-wine font-black uppercase tracking-[0.2em] text-[10px] hover:gap-4 transition-all"
                                >
            ...
            <aside className="lg:w-1/3 space-y-12">
                <div className="bg-vault-darkWine p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border-b-8 border-vault-yellow">
                ...
                <div className="p-10 bg-gray-50 rounded-[3rem] border border-gray-100 shadow-inner">
                    <h4 className="text-xl font-black text-vault-darkWine uppercase tracking-tighter italic mb-8 border-b-2 border-vault-yellow pb-4 inline-block">{content.archiveTitle}</h4>
                    <ul className="space-y-4">
                        {['Academic Reports', 'Newsletters', 'Board Communications', 'Student Journals', 'Digital Policy'].map((cat, i) => (
                            <li key={i}>
                                <button className="w-full flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 hover:border-vault-wine hover:translate-x-2 transition-all group shadow-sm">
                                    <span className="font-black uppercase tracking-widest text-[10px] text-gray-400 group-hover:text-vault-wine">{cat}</span>
                                    <ChevronRight size={14} className="text-gray-300 group-hover:text-vault-wine" />
                                </button>
                            </li>
                        ))}
                    </ul>
      ...
      {/* Simulation Toast */}
      {showDemoAlert && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[300] bg-vault-darkWine text-white px-8 py-4 rounded-full shadow-2xl border-2 border-vault-yellow flex items-center gap-4 animate-fade-in-up">
