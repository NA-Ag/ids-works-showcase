...
const Testimonials: React.FC = () => {
  ...
  const content = {
    ...
    breadcrumbs: [
        ...
        { label: isEn ? "Admissions" : "Admisiones", href: "/demos/integrated/admissions" },
    ...
    <PageLayout title={content.title} breadcrumbs={content.breadcrumbs} bannerImage="https://images.unsplash.com/photo-1524178232363-1fb2b075b655?auto=format&fit=crop&q=80&w=2000">
      <div className="space-y-24 animate-fade-in">
        ...
        <section className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <p className="text-2xl text-vault-darkWine font-black uppercase tracking-tighter italic leading-tight">
                "{content.intro}"
            </p>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {content.categories.map((cat, i) => (
                <div key={i} className="bg-gray-50 p-10 rounded-[2.5rem] border border-gray-100 flex flex-col items-center text-center group hover:border-vault-yellow transition-all">
                    <div className="bg-white p-4 rounded-2xl text-vault-wine mb-6 shadow-sm group-hover:bg-vault-darkWine group-hover:text-white transition-colors">
                        <cat.icon size={32} />
                    </div>
                    <h4 className="text-xl font-black text-vault-darkWine uppercase tracking-tighter italic">{cat.title}</h4>
                </div>
            ))}
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-6xl mx-auto">
            {content.items.map((item, i) => (
                <div key={i} className="bg-white p-12 rounded-[3.5rem] border border-gray-100 shadow-xl relative group flex flex-col justify-between border-b-8 hover:border-idsRed transition-all">
                    <div className="absolute top-10 right-10 text-vault-wine/5">
                        <Quote size={80} />
                    </div>
                    <div className="relative z-10">
                        <div className="inline-flex items-center gap-2 bg-vault-paper px-3 py-1 rounded-full text-[9px] font-black text-gray-400 uppercase tracking-widest mb-8 border border-vault-wine/5">
                            {item.category}
                        </div>
                    ...
                    <div className="pt-8 border-t border-gray-50 flex items-center gap-6 relative z-10">
                        <div className="w-16 h-16 bg-vault-paper rounded-2xl flex items-center justify-center text-vault-wine font-black text-2xl border border-vault-wine/10 uppercase">
                            {item.name.charAt(0)}
                        </div>
                        <div>
                            <h5 className="font-black text-vault-darkWine uppercase tracking-widest text-sm">{item.name}</h5>
        ...
        <div className="bg-vault-darkWine text-white p-12 md:p-24 rounded-[4rem] text-center shadow-2xl relative overflow-hidden border-b-8 border-vault-yellow">
            ...
            <div className="relative z-10 space-y-10 max-w-3xl mx-auto">
                ...
                <Link 
                    ...
                    className="inline-block bg-vault-yellow text-vault-darkWine px-16 py-5 rounded-sm font-black uppercase tracking-widest text-sm hover:bg-white transition-all shadow-xl"
                >
