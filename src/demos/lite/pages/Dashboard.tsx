...
const Dashboard: React.FC = () => {
  ...
  const teamsAssignments = [
    { id: 1, title: "Ensayo de Historia: Soberanía Digital", due: "Feb 28", points: "100/100", status: "Assigned" },
    { id: 2, title: "Examen Matemáticas: Cálculo III", due: "Mar 05", points: "50/50", status: "Pending" },
    { id: 3, title: "Propuesta Feria de Ciencias", due: "Mar 15", points: "10/10", status: "Not Started" }
  ];
  ...
  const getTeamsList = () => {
    ...
    if (originalRole === 'teacher') return [
        { id: '10a', name: "Año 10A - Ciencias", sub: "Grupo de Clase", color: "bg-emerald-600" },
        { id: 'faculty', name: "Hub de Profesores", sub: "Sala de Maestros", color: "bg-vault-blue" },
        { id: 'secondary', name: "Depto Secundaria", sub: "Coordinación", color: "bg-[#8B0000]" }
    ];
    return [
        { id: 'math', name: "Matemáticas", sub: "Materia", color: "bg-blue-600" },
        { id: 'history', name: "Historia Global", sub: "Materia", color: "bg-orange-600" },
        { id: 'arts', name: "Artes Digitales", sub: "Electiva", color: "bg-purple-600" },
        { id: 'class26', name: "Generación 2026", sub: "Comunidad", color: "bg-vault-yellow" }
    ];
  };
        ...
        <div className="flex flex-col h-screen bg-[#f8fafc] font-sans overflow-hidden">
            ...
            <main className="flex-grow overflow-y-auto p-6 md:p-12 space-y-10 max-w-6xl mx-auto w-full">
                <header className="space-y-2">
                    <h2 className="text-4xl font-black text-vault-darkBlue uppercase tracking-tighter italic">Red Institucional</h2>
                    <p className="text-gray-500 font-medium">Conecte con otros egresados y explore oportunidades.</p>
                </header>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white p-8 rounded-[2.5rem] border-2 border-orange-100 shadow-xl space-y-6">
                        <h3 className="text-lg font-black text-vault-darkBlue uppercase tracking-widest">Bolsa de Trabajo</h3>
                        <div className="space-y-4">
                            {[
                                { title: "Junior Software Engineer", company: "TechCorp Mexico", type: "Full-time" },
                                { title: "Coordinador Académico", company: "Colegio IDS", type: "Full-time" }
                            ].map((job, i) => (
                    ...
                    <div className="bg-vault-darkBlue p-8 rounded-[2.5rem] text-white flex flex-col justify-between relative overflow-hidden group">
                        <div className="relative z-10 space-y-4">
                            <h3 className="text-xl font-black text-vault-yellow uppercase tracking-widest">Directorio</h3>
                            <p className="text-sm text-blue-100 opacity-80">Busque egresados por año, carrera o industria.</p>
                            <button className="px-6 py-2 bg-vault-yellow text-vault-darkBlue rounded-lg font-black uppercase text-[10px] tracking-widest">Acceder</button>
                        </div>
        ...
        <div className="flex flex-grow overflow-hidden relative">
          <aside className={`fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-4 border-b border-gray-100 flex justify-between items-center md:hidden bg-gray-50">
                <span className="font-black text-xs text-gray-400 uppercase tracking-widest">Navegación</span>
                <button onClick={() => setIsSidebarOpen(false)} className="text-gray-400 hover:text-[#8B0000] transition-colors"><X size={20} /></button>
            </div>
            <nav className="py-4 px-2 space-y-1">
              {['home', 'users', 'security', 'updates', 'health', 'settings'].map(id => (
                <div key={id} onClick={() => setActiveAdminTab(id)} className={`flex items-center gap-4 p-3 rounded-md cursor-pointer ${activeAdminTab === id ? 'bg-blue-50 text-[#0078d4] font-bold border-l-4 border-[#0078d4]' : 'text-gray-600 hover:bg-gray-100'}`}>
                  {id === 'home' && <Layout size={18} />} {id === 'users' && <Users size={18} />} {id === 'security' && <Shield size={18} />} {id === 'updates' && <FileText size={18} />} {id === 'health' && <Activity size={18} />} {id === 'settings' && <Settings size={18} />}
                  <span className="text-sm capitalize">{id === 'updates' ? 'Actualizaciones' : id}</span>
                </div>
              ))}
            </nav>
          </aside>
          ...
          <main className="flex-grow overflow-y-auto p-6 md:p-12">
            ...
            {activeAdminTab === 'updates' && (
               <div className="bg-white p-10 rounded shadow-sm border border-gray-200 text-center animate-fade-in space-y-6">
                   <h2 className="text-2xl font-black text-vault-darkBlue uppercase tracking-tighter">Actualizaciones Manuales (Versión Lite)</h2>
                   <p className="text-gray-600 max-w-xl mx-auto">En la versión Lite, todas las actualizaciones de contenido, noticias y calendarios se realizan directamente a través del código fuente por nuestro equipo de soporte técnico o el personal asignado por la institución.</p>
                   <div className="bg-blue-50 text-vault-blue p-6 rounded-lg font-mono text-sm inline-block text-left">
                       <p className="font-black mb-2">// Ejemplo de actualización (Requerida en código):</p>
                       <code>git commit -m "Actualización de circulares mensuales"</code><br/>
                       <code>git push origin main</code>
                   </div>
                   <p className="text-sm font-bold text-vault-yellow bg-vault-darkBlue px-6 py-4 rounded-lg inline-block shadow-lg mt-8">
                       🌟 Adquiera la versión ESTÁNDAR para habilitar el Gestor Visual de Contenido (CMS) y publicar como en redes sociales.
                   </p>
               </div>
            )}
            {activeAdminTab !== 'home' && activeAdminTab !== 'updates' && <div className="bg-white p-6 md:p-10 rounded shadow-sm border border-gray-200 animate-fade-in"><h2 className="text-xl font-bold uppercase tracking-tight mb-4">Módulo {activeAdminTab}</h2><div className="h-40 bg-gray-50 flex items-center justify-center border-2 border-dashed border-gray-200 text-gray-400 font-mono text-[10px]">LOG_DEL_SISTEMA: {activeAdminTab.toUpperCase()} ACTIVO</div></div>}
          </main>
        </div>
      </div>
    );
            ...
            <div className="flex-grow overflow-y-auto p-4 md:p-10 max-w-5xl mx-auto w-full space-y-8">
                            ...
                            <div key={post.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex gap-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="pt-2 flex items-center gap-4 text-gray-400">
                                        <button className="flex items-center gap-1 hover:text-[#464775] transition-colors"><MessageSquare size={14} /><span className="text-[10px] font-bold">Responder</span></button>
                                        <span className="text-[10px] font-bold italic">{post.likes} Reacciones</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
                        <table className="w-full text-left">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-400">Tarea</th>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-400">Puntos</th>
                                </tr>
                            </thead>
                ...
                {activeTeamsContent === 'files' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-fade-in">
                        {['Materiales de Clase', 'Manual del Alumno', 'Temario', 'Plantillas'].map((f, i) => (
